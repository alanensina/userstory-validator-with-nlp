...
class SpacyService():
    def processarHistoria(idioma:str, historia:str):
        ...
        if len(sentencas) > 1 and not utils.verifica_erro_separacao(sentencas):
            ...
            if erros == None:
                ...
                return ResponseHistoria(historia, Constantes.SPACY, tempo, bem_formada, atomica, minima, None, None, None, tags, erros)
        else:
            end = timeit.default_timer()  
            tempo = utils.formatar_tempo(start, end)
    ...
    def processarCenario(idioma:str, cenario:str):
        ...
        if len(sentencas) > 1 and not utils.verifica_erro_separacao(sentencas):
            ...
            if erros == None:
                return ResponseCenario(cenario, Constantes.SPACY, tempo, bem_formada, atomica, minima, precondicao, acao, finalidade, tags, erros)
            else:
                return ResponseCenario(cenario, Constantes.SPACY, tempo, bem_formada, atomica, minima, None, None, None, tags, erros)
            
        else:
            end = timeit.default_timer()  
            tempo = utils.formatar_tempo(start, end)
            return utils.retorna_erro_historia(cenario, Constantes.SPACY, sentencas, tempo)
